...
class MY_Session extends CI_Session {
	...
	private function _get_user_information($room='admin') {
		...
		elseif($room='club') {
			$club_id = $this->userdata('club_id');
			if ($club_id != FALSE) {
				$this->CI->db->where('club_id' , $club_id);
				$res = $this->CI->db->get('club');
				$result = $res->row_array();
				return $result;
			} else { return FALSE; }
		}
	...
	public function validate_club() {
		...
		// Check if user is logged in
		$sess_id = $this->userdata('club_id');
		//print $sess_m_id;

		if ($sess_id) {
			//$this->m_id = $sess_id;
			$this->userinfo = $this->_get_user_information('club');
	...
	public function get_user_info($room='admin') {
		...
		switch($room){
			...
			case 'member'	:
				...
				break;
			case 'club'	:
				$sess_uid = $this->userdata('club_id');
				//$this->m_id = $sess_uid;
				$this->userinfo = $this->_get_user_information('club');
				$this->CI->sci->assign('userinfo', $this->userinfo);
				$this->CI->sci->assign('LOGGEDIN', '1');
	...
	*/
	...
	/**
	* Push confirm to ajax
	* @
	*/
	function push_confirm($type='0', $msg='') {
		if($type == 1) {
			$class = 'success';
			$msg = $msg?$msg:'Operation Success !';
		} else {
