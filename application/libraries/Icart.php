...
class Icart {
	...
	function update_quantity($cart_id=0, $new_cart_qty=0) {
		...
		$this->CI->db->where('p_status' , 'Active');
		...
		//kalau perubahan lebih besar dari quantity sekarang, batalkan
		if($diff > $pq['pq_quantity']) { return 'out of stock'; }

		$this->CI->db->trans_start();
			//update cart
			$subtotal = $cart['cart_price'] * $new_cart_qty;
			$totalweight = $p['p_weight'] * $new_cart_qty;
			$this->CI->db->where('cart_id' , $cart_id);
			$this->CI->db->set('cart_totalweight' , $totalweight);
	...
	function insert( $p_id=0, $pr_id=0, $pq_id=0, $quantity=0, $type="Product") {
		...
		if($this->use_cookies == TRUE){
			...
			if($existing) {
				...
				$totalweight = $weight * $new_quantity;
				$this->CI->db->where('cart_id' , $cart_id);
				$this->CI->db->set('cart_totalweight' , $totalweight);
