...
class Menu extends MY_Controller {
	...
	function load($mp_id = 0 , $parent_id = 0, $lv = 0) {
		...
		foreach($res->result() as $k=>$row) {
			echo '<li class="'.$row->m_status.' menulv_'.$lv.'">';

			echo "<div class='menu_actions_top'><a href='#' id='{$row->m_id}' class='edit_menu'>Edit</a> | <a href='#' id='{$row->m_id}' class='delete_menu' title='Delete this'>Delete</a>  |  ";
			if($row->m_status == 'Active') {
				echo "<a href='#' id='{$row->m_id}' class='suspend_menu' title='Suspend this'>Suspend</a>";
			} else {
				echo "<a href='#' id='{$row->m_id}' class='activate_menu' title='Activate this'>Activate</a>";
			}
			echo "</div>";
			echo "<div class='menu_detail'>";
			echo "<div class='menu_name'>".$row->m_name."</div>";
			echo "<div class='menu_link'>".$row->m_link."</div>";
			echo "</div>";
			$this->load($mp_id , $row->m_id, ($lv+1));
			echo "</li>";
		}
		echo "</ul>";

		if ($parent_id > 0) {
			echo "<div class='menu_actions_bottom'>";
