...
class Site_config extends MY_Controller {
	...
	function site() { 
		...
		$this->form_validation->set_rules('config[]','','trim'); 
	...
	function configuration() { 
		...
		$this->form_validation->set_rules('config[]','','trim'); 
		if($this->form_validation->run() == FALSE ) {
			$this->sci->assign('currpage' , 'configuration');
			$this->load_sidebar();
			$this->sci->da('configuration.htm');
		} else {
			$config = $this->input->post('config');
			foreach($config as $k=>$tmp) {
				$this->db->where('c_key' , $k);
			...
			$this->session->set_confirm(1);
			redirect($this->mod_url."configuration");
		}
	}
	...
	function elements() { 
		...
		$this->load->library('form_validation');
		$this->form_validation->set_rules('image[]','','trim'); 
		$this->form_validation->set_rules('image_remove[]','','trim'); 
		$this->form_validation->set_rules('image_default[]','','trim'); 
		$this->form_validation->set_rules('config[]','','trim'); 
		if($this->form_validation->run() == FALSE ) {
	...
	function validation_setting() {
		$this->form_validation->set_rules('c_text_key[]', 'key', 'trim');
		$this->form_validation->set_rules('c_text_value[]', 'value', 'trim');
		$this->form_validation->set_rules('c_image_key[]', 'key', 'trim');
		$this->form_validation->set_rules('c_image_value[]', 'value', 'trim');
		$this->form_validation->set_rules('c_cat', 'category', 'trim');
	}
