...
class Page extends MY_Controller {
	...
	function view($c_id=0, $c_slug='' ) {
		...
		if($content['c_code'] == 'about-us') {
			$this->db->where('c_code' , 'about-us-sidebar');
			$res = $this->db->get('content');
			$aboutus_sidebar = $res->row_array();
			$this->sci->assign('aboutus_sidebar' , $aboutus_sidebar);
		}

		if($content['c_code'] == 'services') {
			$this->db->where('s_status' , 'Active');
			$this->db->order_by('s_date' , 'DESC');
			$this->db->limit(8,0);
			$res = $this->db->get('startup');
			$startups = $res->result_array();
			$this->sci->assign('startups' , $startups);
		}
