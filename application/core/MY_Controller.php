...
class MY_Controller extends CI_Controller {
	...
	function _crop_image($source_file='', $target_thumb='', $width=100, $height=100 ) {  
		$config['width'] = $width*2;
		$config['height'] = $height*2;
		$config['image_library'] = 'gd2';
		$config['source_image'] = $source_file;
		$config['new_image'] = $target_thumb;
		if($width > $height) {
		...
		$size = getimagesize($target_thumb);
		$width_temp = $size[0];
		$height_temp = $size[1]; 
		//print $width_temp."-".$height_temp.":";
		 
		$top_offset = ($height_temp/2) - ($height/2);
		$left_offset = ($width_temp/2) - ($width/2);  
		$config['image_library'] = 'gd2';
		$config['source_image'] = $target_thumb;
		$config['new_image'] = $target_thumb;
		$config['width'] = $width;
	 ...
	 *
	 */
	function ajax_list_select($pagenum = 1 ,
							  $pagelimit = '20',
							  $orderby='',
							  $ascdesc='DESC',
							  $encodedkey='') {
