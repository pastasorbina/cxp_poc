	...
	slide:function(nextslide, dir){ //possible values for dir: "left", "right", "top", or "down"
		...
		this.$imageslides.eq(nextslide).show().css(createobj([posprop, nextslide_initialpos], ['opacity', 0.5])) //show upcoming slide
			.stop().animate(createobj([posprop, 0]), setting.slideduration, function(){
				...
				.find('div.desc').stop().fadeIn()
				slider.animation_isrunning=false
			})
			.find('div.desc').hide()

		//WILL *CHANGE NAV CLASS
		$('.bgcar_nav_btn').removeClass('active');
		$('.bgcar_nav_btn').addClass('inactive');
		var navid = this.setting.imagearray[this.curslide][2];
		//console.log(navid);
		$('.bgcar_nav_btn.nbtn_'+navid).removeClass('inactive').addClass('active');
